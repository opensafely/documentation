...
Note: Users with Apple Silicon computers need to [enable Rosetta emulation](/install-docker/#macs-with-an-apple-silicon-processor). Note for Docker Desktop versions between 4.16.0 and 4.24.4 this setting is found in Settings under the _Features in development_ and then _Beta features_ tab.
