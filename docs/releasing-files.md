...
#### Midpoint 6 rounding
By rounding to midpoint 6, we make sure that the numerator and denominator of our rate do not break our [suppression rules](https://docs.opensafely.org/releasing-files/#redacting-counts-less-than-or-equal-to-5). The method has desirable properties such as: its unbiasedness; zero-are-zero’s; of a non-zero rounded value of `x`, we know true values range between `x-2` and `x+3` (for more information and an explanation of why these properties could be important, we refer to our 'note' box below).
