...
## Extracting ethnicity across multiple time points

Querying ethnicity can take a long time as a result of having to map from individual codes to broad categories (i.e the mapping of individual codes in [this codelist](https://www.opencodelists.org/codelist/opensafely/ethnicity/2020-04-27/#full-list) into either 1 of 6 or 16 possible categories). This is ok if you only extract ethnicity at one time point, but if you are running your study definition across multiple time points it can slow down the cohort extraction. As ethnicity is generally expected to remain constant over time, you can instead extract ethnicity once (taking the latest recorded ethnicity code for each patient) and then map these to the files generated by your main study definition for each time point.

An example study definition for extracting ethnicity is shown below. Note that the study population should contain **all** patients.

`study_definition_ethnicity.py`
```py
    study = StudyDefinition(
        default_expectations={
...
You can then join the outputs of this study definition using a function like this:
...
```py
    ...
    """Finds 'input_ethnicity.feather' in directory and combines with each input file."""
        ...
        # map ethnicity using the patient_id
        ethnicity_dict = dict(zip(ethnicity_df['patient_id'], ethnicity_df['ethnicity']))
        
        # loop over files in directory and overwrite with a file containing ethnicity
