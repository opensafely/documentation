...
Querying ethnicity can take a long time as a result of mapping from individual codes to broad categories. This is ok if you only extract ethnicity at one time point, but if you are running your study definition across multiple time points it can take slow down the cohort extraction. As ethnicity is generally expected to remain constant over time, you can instead extract ethnicity once at the end of the study period and then map these to the files generated by your main study definition for each time point.
